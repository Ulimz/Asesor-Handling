...
def backup_sector():
    print("💾 BACKING UP CONVENIO SECTOR DATA...")
    ...
    backup_path = os.path.join(os.getcwd(), 'backend', 'backups', f'sector_backup_{timestamp}.json')
    ...
    try:
        ...
        docs = db.query(LegalDocument).filter(LegalDocument.company == "general").all()
        for d in docs:
